 ...
 * MQTT em vez de HDLC/serial (ver Integration/MqttFleetLink.ts e a seção "Conexão com o
 * RobotSwarmSimulator" do AGENTS.md). Espelha `src/Protocols/Mari/` do backend.
 *
 ...
 * motor de campos genérico (`Protocol.Codec.ts`) usado pelos payloads menores.
 */
...
/** Address hex (16 chars) → 8 bytes little-endian (também usado pelos eventos de borda, ver DotBot.Payload.ts). */
...
// formato usado nos tópicos MQTT `to_edge`/`to_cloud` (ver EdgeEvent.enum.ts).
